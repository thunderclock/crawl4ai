"""
Minio Storage Manager for RedNote Browser State

This module provides functionality to save and load browser state (cookies, localStorage, etc.)
to/from Minio object storage.
"""
...
class MinioStorageManager:
    """Manages browser state storage in Minio"""
    ...
    def __init__(
        ...
        access_key: str,
        secret_key: str,
        bucket_name: str = "rednote-browser-state",
        secure: bool = True,
        ...
        Args:
            ...
            bucket_name: Bucket name for storing browser state (default: 'rednote-browser-state')
    ...
    def _get_storage_state_key(self, profile_name: str = "default") -> str:
        """Get object key for storage state JSON"""
        return f"profiles/{profile_name}/storage_state.json"
    
    def _get_browser_data_key(self, profile_name: str = "default") -> str:
    ...
    async def save_storage_state(
        ...
        """
        Save browser storage state (cookies, localStorage) to Minio
        
        Args:
            storage_state: Storage state dictionary from Playwright context.storage_state()
            profile_name: Profile name to identify the browser state
    ...
    async def save_browser_data_dir(
        ...
        """
        Save entire browser data directory to Minio as a zip archive
        ...
        try:
            ...
            # Upload to Minio
            key = f"{self._get_browser_data_key(profile_name)}/browser_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
            self.client.put_object(
                bucket_name=self.bucket_name,
                object_name=key,
            ...
            latest_key = f"{self._get_browser_data_key(profile_name)}/browser_data_latest.zip"
            self.client.put_object(
                bucket_name=self.bucket_name,
            ...
            self._log(f"Saved browser data directory to Minio: {key}")
            return True
        except Exception as e:
    ...
    async def load_browser_data_dir(
        ...
        Args:
            ...
            profile_name: Profile name to identify the browser state
            use_latest: If True, load latest version, otherwise load most recent timestamped version
        ...
        try:
            ...
            if use_latest:
                key = f"{self._get_browser_data_key(profile_name)}/browser_data_latest.zip"
            else:
                # List all browser data archives and get the most recent
                prefix = f"{self._get_browser_data_key(profile_name)}/browser_data_"
